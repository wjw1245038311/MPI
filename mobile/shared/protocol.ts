/**
 * Vendored copy of src/main/remote/protocol.ts (transport-agnostic protocol v1
 * envelope + remote data types) for the mobile PWA — docs/MOBILE-DESIGN.md §6.1.
 *
 * Keep in sync with the desktop source; scripts/test-pwa-shared.mjs asserts the
 * two implementations behave identically, so drift fails CI.
 */
 ...
 * 手机端发送的任意文件附件（base64）。
 *
 * 与图片不同：图片会作为模型的图片输入直传，文件则先由主机落盘（stageClipboardFile，
 * 与桌面粘贴文件同一目录），再按桌面的 processAttachments 规则处理——文本类
 * 小文件内联进提示，二进制/大文件以 <file path=… /> 引用交给 agent 自己读。
 */
