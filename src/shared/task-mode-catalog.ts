/**
 * 任务模式目录（task-mode catalog）—— main 与 renderer 的单一来源。
 *
 * 为什么放在 src/shared：手机端（remote snapshot 的 availableModes）与桌面端
 * （模式下拉）必须看到同一份模式定义；而内置模式的**参数与指令正文**曾经只
 * 存在于渲染进程内存里（config.taskModes 里往往没有它们，甚至残留已废弃 id），
 * 主进程无从得知「迭代模式」是什么。这里一次定义，两侧引用。
 *
 * 纯数据 + 纯函数，不依赖 electron / DOM。
 */

/** 权限级别（与 src/main/config.ts、renderer/lib/types.ts 结构一致）。 */
export type PermissionLevel = "readonly" | "strict" | "sandbox" | "full";

/** 一个任务模式预设：权限 + 思考等级 + 行为指令/说明书 + 强制只读底线。 */
