...
export function qaModeInstruction(mode: "inline" | "manual", language: "zh" | "en" = "zh"): string {
  ...
  return zh
    ? "## 问答方式：内联快速选择\n当你向用户提出带明确选项的问题（每题 2-6 个选项）时，在回复中输出一个 choices 围栏块：单独一行 ```choices，下一行 JSON 数组 [{\"title\":\"问题\",\"options\":[\"A\",\"B\"]}]（option 可为字符串或 {\"label\":\"…\",\"detail\":\"一句说明\"}），再单独一行 ```。MPI 会把它渲染成可点击的内联面板；用户点选后以一条「我的选择：1. … → …」消息回复，按题号处理即可。若其它指令（如访谈类 skill）要求用 ❓/编号文本呈现带选项的问题，同样改用 choices 围栏块呈现这些选项；上下文与分析仍放在围栏前的普通文字里。没有明确选项的开放性问题用普通文本提问。"
    : '## Q&A style: inline quick choice\nWhen you ask the user questions with discrete options (2-6 per question), emit a choices fence in your reply: a line ```choices, then a JSON array [{"title":"question","options":["A","B"]}] (option = string or {"label":"…","detail":"one-line note"}), then a closing ```. MPI renders it as a clickable inline panel; the user\'s picks come back as one message starting with "My choices:" — handle them by question number. If another instruction (e.g. an interviewing skill) prescribes ❓/numbered text for option-bearing questions, use the choices fence for those options instead; keep context and analysis in normal text before the fence. Open-ended questions stay plain text.';
}
