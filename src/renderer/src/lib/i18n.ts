...
const exact: Record<string, string> = {
  ...
  "更新项目路径…": "Update project path…",
  "项目文件夹被移动后，把会话与配置指向新位置": "Re-point sessions and settings after the project folder moved",
  ...
  "新位置与当前项目路径相同": "New location is the same as the current project path",
  "所选位置不存在或不是文件夹": "Selected location does not exist or is not a folder",
  ...
  "新建会话的初始提供商；写入 settings.json（~/.pi/agent），与终端 pi 共享。": "Initial provider for new sessions; written to settings.json (~/.pi/agent) and shared with terminal pi.",
  ...
  "会话已移入回收站，可在设置「数据管理」中恢复或永久删除。": "Session moved to trash. Restore or permanently delete it from Settings → Data management.",
