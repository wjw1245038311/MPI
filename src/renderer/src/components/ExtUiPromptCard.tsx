...
import { choiceOptions } from "../lib/choice";
import { useStore } from "../store";
import { ChevronRight, Close, Shield } from "./icons";
...
export function ExtUiPromptCard({ threadId }: { threadId: string }) {
  ...
  // Per-option “细节/Detail” expansion.
    ...
    <div className={`extui-card ${request.method} ${isSandbox ? "sandbox-card" : ""}`} role="alertdialog" aria-labelledby={`extui-title-${request.id}`}>
      <div className="extui-card-head">
        ...
        <button
          ...
          onClick={cancel}
          title={language === "zh" ? "拒绝并关闭" : "Deny and close"}
        >
          <Close size={14} />
        </button>
      </div>
      ...
      {request.method === "confirm" ? (
        ...
        <div className="extui-card-options">
          {choiceOptions(request.options).map((opt, index) => {
            ...
            // mode-switch cards lead with the approve option — badge it too.
